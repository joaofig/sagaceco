...
class WeeklyLogModel(val radius: Double) {
  ...
  def getValue(record: LogRecord): Double = models(getIndex(record)).getValue(record.period)
  
  private def getIndex(record: LogRecord): Int = record.period % 2016
}
...
class LinearRegressionModel {
  ...
  def getValue(x: Double): Double = alpha + beta * x
  ...
  private def getResidualSumOfSquares(): Double = beta * beta * sumXX - 2 * beta * sumXY + 2 * alpha * beta * sumX + sumYY - 2 * alpha * sumY + alpha * alpha * count
  
  private def getResidualVariance(): Double = getResidualSumOfSquares / (count - 2)
}
